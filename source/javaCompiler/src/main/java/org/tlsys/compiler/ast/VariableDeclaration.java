...
import org.tlsys.compiler.UnhandledCompilerProblemException;
...
public class VariableDeclaration extends ASTNode
{
    ...
    public static String getLocalVariableName(Method method, int slot, int pc)
    {
        if (method.getLocalVariableTable() != null)
        {
            ...
            {
                lvar= table[i];
                if (lvar.getIndex() == slot && lvar.getStartPC()-1 <= pc && pc <= lvar.getStartPC() + lvar.getLength())
                {
    ...
    private static boolean isSuperpositionDetected(LocalVariable[] table, LocalVariable lvar)
    {
        ...
        {
            if (lvar != localVariable && lvar.getName().equals(localVariable.getName()) && lvar.getStartPC()-1 >= localVariable.getStartPC() && (lvar.getStartPC() + lvar.getLength() <= localVariable.getStartPC() + localVariable.getLength()))
                return true;
        }
        return false;
    }
